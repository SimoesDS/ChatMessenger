...
//          System.out.println("EU SOU O " + Core.getUserSession().getNome() + " PARA O USUARIO DO ID " + targetId);
          //  System.out.println("Eu sou: " + Core.getUserSession().getNome() + " enviei para: " + targetId);
          new Thread(new ClientReply("localhost", 5056, Core.getUserSession(), messageContainer.getText(), targetId)).start();
  ...
  class HandlerListener implements AlertaTelaListener, ICommands {
    ...
    public void AlertaTela(RequestResponseData requestResponseData) {
      switch (requestResponseData.getCommand()) {
        case AUTHENTICATED:
          Core.setUserSession((Usuario) requestResponseData.getObject());
          Utils.setUSerSession();
...
//          System.out.println("BodyPanel recebeu a mensagem: " + (String) requestResponseData.getObject());
          ...
          Core.addChatMessage((String) requestResponseData.getObject(), "in");
          break;
        case UNREGISTERED:
          showErrorMsg("Login ou senha inválidos");
          break;
